---
title: "Learn Collaborate Build"
---

```{=html}
<div id="post-images" class="owl-carousel">
    <div class="item">
        <img src="/media/flying-book-in-library.jpeg" alt="Book flying through a library">
    </div>
    <div class="item">
        <img src="/media/team-at-beach.jpeg" alt="A team group of people at the beach at sunset">
    </div>
    <div class="item">
        <img src="/media/blueprints.jpeg" alt="Blueprints with pencil and metal ruler">
    </div>
</div>

<!-- Include Owl Carousel CSS -->
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/OwlCarousel2/2.3.4/assets/owl.carousel.min.css">

<!-- Include jQuery -->
<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>

<!-- Include Owl Carousel JavaScript -->
<script src="https://cdnjs.cloudflare.com/ajax/libs/OwlCarousel2/2.3.4/owl.carousel.min.js"></script>

<script src="/scripts.js"></script>
```

**Let's Do Digital** has been a centre for safe and open discussion and learning around digital health since 2022. We have had two great conferences and several webinars to date. 

## Updates

* We have had lots of interest for an evening coding session. So we built one. See details [here](/tickets/programming-in-healthcare-module-1-July-2024.qmd)
* Tickets are also open for the first (of many) of the "Ask a Clinician" series. Find out more [here](/tickets/ask-a-clinician-1.qmd).
* Our first **{{< var coding-series-title >}} course** is now open to registration. Please find further details about the course and registration [here](/tickets/programming-in-healthcare-module-1-June-2024.qmd).



Look out for another series of learning opportunities. We are planning to open the floor to computer scientists, data scientists, cybersecurity specialists, basically anyone in the digital domain who works in or around healthcare, to ask a clinician about their workflows, workloads, and thought processes. We hope to enable those in digital fields to better understand the data and information in front of them by granting them direct access to clinicians on the front line.

```{=html}
<div class="supporter-frame">
    <h3>Individual supporters</h3>
    If you wish to support us, please follow the Sponsor link in the navigation bar above.
</div>
<br>
<div class="supporter-frame">
    <h3>Healthcare group supporters</h3>
    If you wish to support us, please follow the Sponsor link in the navigation bar above.
</div>
```